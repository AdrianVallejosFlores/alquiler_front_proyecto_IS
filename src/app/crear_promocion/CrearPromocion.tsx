...
const CrearPromocion: React.FC<CrearPromocionProps> = ({ onBack, onSave }) => {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto bg-white rounded-xl shadow-lg space-y-8">
      ...
      {/* Título */}
      <h1 className="text-3xl font-bold text-blue-600">Crear nueva promoción X.</h1>
      ...
      <div>
        ...
        <textarea
          ...
          className={`w-full px-3 py-2 text-gray-700 border ${isOverLimit ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-150 ease-in-out resize-none`}
          placeholder="Escribe la descripción de la promoción aquí..."
        />
        
        {/* Contador de caracteres (DEBE APARECER) */}
        <div className={`text-sm mt-1 text-right ${isOverLimit ? 'text-red-500 font-bold' : 'text-gray-500'}`}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-8">
          ...
          {/* Opción SI - Estilo de radio button estándar (parcialmente lleno) */}
          <label className="flex items-center cursor-pointer">
            <input 
              ...
              value="Si" 
              checked={activa === true} 
              onChange={() => {setActiva(true); setError(null);}} 
              // Estilo estándar de radio button
              className="h-5 w-5 text-blue-600 border-gray-300 focus:ring-blue-500"
            />
            <span className="ml-2 text-lg text-gray-700">Si</span>
          </label>

          {/* Opción NO - Estilo de radio button estándar (parcialmente lleno) */}
          <label className="flex items-center cursor-pointer">
            <input 
              type="radio" 
              name="activa" 
              ...
              onChange={() => {setActiva(false); setError(null);}} 
              // Estilo estándar de radio button
              className="h-5 w-5 text-blue-600 border-gray-300 focus:ring-blue-500"
            />
