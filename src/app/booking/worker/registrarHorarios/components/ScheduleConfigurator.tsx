...
const ID = "6930e18bad0e1c3cfdd220f8";
...
export default function ScheduleConfigurator() {
    ...
    const generateSummary = (data: ScheduleConfig | WeeklyUniformSchedule, type: 'daily' | 'weekly'): SummaryData => {
        if (type === 'daily') {
            ...
            return { title: "Configuración Diaria Guardada (Híbrida)", lines };
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const diasMap: Record<string, number> = { Lunes:1, Martes:2, Miercoles:3, Jueves:4, Viernes:5, Sabado:6, Domingo:7 };
        let payload: any = {};
        ...
        const endpoint = `${API_URL}/api/devcode/proveedores/${ID}/horarioLaboral`;
            ...
            <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 transition-opacity duration-300">
                <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-lg transform scale-100 transition-transform duration-300">
                    ...
                    <div className="space-y-2 mb-6 p-4 bg-gray-50 rounded border border-green-200 overflow-y-auto max-h-60">
                        {lines.map((line, index) => <p key={index} className="text-sm text-gray-800 font-medium">{line}</p>)}
                    </div>
                    <div className="flex justify-end">
                        <button onClick={() => setIsSummaryModalOpen(false)} className="px-6 py-2 text-sm font-semibold text-white bg-blue-900 rounded-md hover:bg-blue-800">Entendido</button>
                    </div>
                </div>
            </div>
        );
    ...
    const WeeklyConfigSection = () => {
        ...
        return (
            <section className="space-y-6 bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Selecciona los días para aplicar este horario</label>
                    <div className="flex flex-wrap gap-2">
                        {DAYS_OF_WEEK.map(day => {
                            const isSelected = selectedDays.includes(day);
                            return (
                                <button key={day} type="button" onClick={() => toggleWeeklyDay(day)} className={`py-2 px-4 text-sm font-medium rounded-full transition-colors duration-150 ${isSelected ? 'bg-blue-900 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                                    {day}
                                </button>
                            );
                        })}
                ...
                <hr className="my-4" />
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Rangos de Horario (Se aplican a todos los días seleccionados)</label>
                    ...
                    {ranges.map((range, index) => {
                        ...
                        return (
                            <div key={index} className={`flex items-center space-x-3 mb-3 ${isRangeError || isGeneralRangeError ? 'p-2 rounded bg-red-50' : ''}`}>
                                <input type="time" value={range.start} onChange={(e) => handleWeeklyTimeChange(index, 'start', e.target.value)} className={`form-input w-32 text-center rounded-md border-gray-300 shadow-sm ${isRangeError || isGeneralRangeError ? 'border-red-500' : ''}`} />
                                <span className="text-gray-500 text-lg font-bold">-</span>
                                <input type="time" value={range.end} onChange={(e) => handleWeeklyTimeChange(index, 'end', e.target.value)} className={`form-input w-32 text-center rounded-md border-gray-300 shadow-sm ${isRangeError || isGeneralRangeError ? 'border-red-500' : ''}`} />
                                {ranges.length > 1 && (
                                    <button type="button" onClick={() => removeRangeWeekly(index)} className="text-red-500 hover:text-red-700 transition duration-150">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3m-5 0h14"></path></svg>
                                    </button>
                                )}
                    ...
                    <button type="button" onClick={addRangeWeekly} className="text-sm text-blue-700 hover:text-blue-900 mt-2 flex items-center transition duration-150">+ Añadir Rango</button>
                </div>
            </section>
        );
    };
    ...
    return (
        <main className="min-h-full p-6 bg-white font-sans">
            <div className="max-w-4xl mx-auto">
                <a href="/booking/worker" className="inline-block px-4 py-2 text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">Volver</a>
                <header className="text-center mb-8">
                    <h1 className="text-4xl font-bold text-blue-900">Configuración de Horarios Laborales</h1>
                </header>
                <div className="flex border-b border-gray-200 mb-6">
                    <button className={`py-2 px-4 text-sm font-medium transition-colors duration-200 ${activeTab === 'daily' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`} onClick={() => setActiveTab('daily')}>Configuración Diaria (Híbrida)</button>
                    <button className={`py-2 px-4 text-sm font-medium transition-colors duration-200 ${activeTab === 'weekly' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`} onClick={() => setActiveTab('weekly')}>Configuración Semanal (Uniforme)</button>
                </div>
                                        ...
                                        <div key={day} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                                            <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                                                <div className="flex items-center space-x-3">
                                                    <input type="checkbox" id={`${day}-enabled`} checked={isEnabled} onChange={() => toggleDay(day)} className="h-4 w-4 text-blue-900 border-gray-300 rounded focus:ring-blue-900" />
                                                    ...
                                                    {daySchedule.ranges.map((range, index) => {
                                                        ...
                                                        return (
                                                            <div key={index} className={`flex items-center space-x-3 mb-3 ${isDayError || isRangeError ? 'p-2 rounded bg-red-50' : ''}`}>
                                                                <input type="time" value={range.start} onChange={(e) => handleTimeChange(day, index, 'start', e.target.value)} className={`form-input w-28 text-center rounded-md border-gray-300 shadow-sm ${isDayError || isRangeError ? 'border-red-500' : ''}`} />
                                                                <span className="text-gray-500">-</span>
                                                                <input type="time" value={range.end} onChange={(e) => handleTimeChange(day, index, 'end', e.target.value)} className={`form-input w-28 text-center rounded-md border-gray-300 shadow-sm ${isDayError || isRangeError ? 'border-red-500' : ''}`} />
                    ...
                    <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg shadow-2xl w-96">
                            ...
                            <div className="flex justify-end space-x-3">
                                ...
                                <button onClick={() => { setSchedule(initialSchedule); setWeeklySchedule(initialWeeklySchedule); setErrors({}); setHasChanges(false); setIsModalOpen(false); }} className="px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-md hover:bg-red-700">Sí, Descartar Cambios</button>
                            </div>
                        </div>
                    </div>
                )}
