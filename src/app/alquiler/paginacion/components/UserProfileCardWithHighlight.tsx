...
interface UserProfileCardWithHighlightProps {
  usuario: UsuarioResumen;
  onContactClick?: () => void;
  onViewDetails?: (id: string | number) => void;
}

export const UserProfileCardWithHighlight: React.FC<UserProfileCardWithHighlightProps> = ({ 
  usuario, 
  onContactClick, 
  onViewDetails 
}) => {
      ...
      <div className="flex flex-row w-full">
        ...
        <div className="w-1/3 min-w-[220px] bg-gradient-to-r from-blue-600 to-blue-500 p-6 flex flex-col justify-center">
          <h2 className="text-2xl font-bold text-white mb-2 leading-tight">
            {shouldHighlight ? (
              <HighlightedText text={nombre} searchTerms={searchTerms} highlightClassName="bg-yellow-500 text-black" />
            ) : (
              nombre
            )}
          </h2>
          ...
          {ciudad && (
            <div className="flex items-center text-white/90 mb-2">
              ...
              <span className="text-base">
                {shouldHighlight ? (
                  <HighlightedText text={ciudad.nombre} searchTerms={searchTerms} highlightClassName="bg-yellow-500 text-black" />
                ) : (
                  ciudad.nombre
                )}
              </span>
              ...
              <div className="flex flex-wrap gap-2">
                {especialidades.map((esp) => (
                  <span key={esp.id_especialidad} className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium border border-blue-200">
                    {shouldHighlight ? (
                      <HighlightedText text={esp.nombre} searchTerms={searchTerms} />
                    ) : (
                      esp.nombre
                    )}
                  </span>
            ...
            <div className="mb-2">
              ...
              <div className="bg-gray-50 rounded px-3 py-2 text-gray-700 text-sm flex justify-between items-center">
                <span>
                  {shouldHighlight ? (
                    <HighlightedText 
                      text={servicios.map(s => s.nombre).join(', ')} 
